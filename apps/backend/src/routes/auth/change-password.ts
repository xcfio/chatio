import { AuthenticatedUser, ChangeUserPassword, ErrorResponse } from "@repo/schema"
...
export default function ChangePassword(fastify: Awaited<ReturnType<typeof main>>) {
    fastify.route({
        method: "PATCH",
        ...
        handler: async (request, reply) => {
            try {
                ...
                if (!timingSafeEqual(Buffer.from(oldUser.password), Buffer.from(HmacPassword(oldPassword)))) {
